...
- [ ] Phase 4 complete — README with numbers, demo live, tag `v0.1.0` — *2026-09-18: three days run (structural counters 0 on all three; the queue finding replicates), README + `crossday.md` + `docs/setup.md` + vault copy in `docs/`, repo pushed, `v0.1.0` tagged, CI green, 97 GB of data cleared with the small artifacts kept in the repo. **Two things left, both mine: flip Settings -> Pages -> Source: GitHub Actions (one click, then the demo is live), and record the 3-minute screencast from `docs/screencast.md`.** 2026-09-20: day 3's spread/OFI cells are no longer empty — all three days were re-downloaded and re-run on a second machine (Linux) that had the disk for the conversion, reproducing every counter byte-identically; see the daily log and D33*
