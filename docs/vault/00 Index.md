...
- [x] Phase 4 complete — README with numbers, demo live, tag `v0.2.0` — *2026-09-20: demo is live at <https://phamhongphuc2712.github.io/order-book/>, all three days carry spreads and OFI, the pipeline runs on Linux and Windows, `phuc` merged to `main`, CI green, `v0.2.0` tagged. The screencast is storyboarded shot by shot with every spoken number traced to its generated file and every command dry-run — recording it is the one remaining act, and it is mine.* — earlier: *2026-09-18: three days run (structural counters 0 on all three; the queue finding replicates), README + `crossday.md` + `docs/setup.md` + vault copy in `docs/`, repo pushed, `v0.1.0` tagged, CI green, 97 GB of data cleared with the small artifacts kept in the repo. **Two things left, both mine: flip Settings -> Pages -> Source: GitHub Actions (one click, then the demo is live), and record the 3-minute screencast from `docs/screencast.md`.** 2026-09-20: day 3's spread/OFI cells are no longer empty — all three days were re-downloaded and re-run on a second machine (Linux) that had the disk for the conversion, reproducing every counter byte-identically; see the daily log and D33*
